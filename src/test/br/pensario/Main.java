...
public class Main {
	...
	private static NCLBody montaBody(NCLHead head) throws Exception {
		...
		NCLNode node = new NCLMedia("mcsaa");		
		NCLRole role = new NCLConditionRole(NCLDefaultConditionRole.ONBEGIN);		
		
		NCLBind bind1 = new NCLBind(role,node);
		
		NCLBind bind2 = new NCLBind(role,node);
